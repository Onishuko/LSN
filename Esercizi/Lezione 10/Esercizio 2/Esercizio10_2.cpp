...
int main(int argc, char* argv[]){
    ...
    //Read cities coordinate from file.dat
    ...
    //Initializing best cost and path per rank
    vector<double> cost;
    vector<double> best_prog;
    cost.reserve(n_steps);
    best_prog.reserve(n_steps);

    double current_best = replica.path.cost;
    ...
    for(int step = 0; step < n_steps; step++){
        ...
        //Save current cost after possible exchange
        cost.push_back(replica.path.cost);

        //Update best solution found by this rank
        if(replica.path.cost < best.cost) best = replica.path;

        //Update progressive best
        if(replica.path.cost < current_best) current_best = replica.path.cost;
    ...
    vector<double> best_half(cost.size(), 0.0);
    string filename = "PT_rank_" + to_string(rank) + ".dat";

    tsp.print(filename, cost, best_prog, best_half);

    string path_filename = "PT_best_path_rank_" + to_string(rank) + ".dat";

    cout << "Printing best path for rank " << rank << endl;

    tsp.print_best_path(path_filename, best);

    cout << "Rank " << rank << " succesfully printed on file" << endl;
