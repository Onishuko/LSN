...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double local_field = _J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H; 
    double p_plus = 1.0 / (1.0 + exp(-2.0 * _beta * local_field));
    if(_rnd.Rannyu() < p_plus) _particle(i).setspin(1);
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _restart;
      coutf << "RESTART= " << _restart << endl;
      /*if(_sim_type == 0 && _restart == 0){
        input >> _start_velocities;
        coutf << "START_VELOCITIES= " << _start_velocities << endl;
      }*/
    } else if( property == "REVERSE" ){
      input >> _reverse;
      coutf << "REVERSE= " << _reverse << endl;
    }else if(property == "VELOCITY_DISTRIBUTION"){
      input >> _start_velocities;
      if(_restart == 0){
        coutf << "VELOCITY_DISTRIBUTION= " << _start_velocities << endl;
      } else {
        coutf << "VELOCITY_DISTRIBUTION= ignored (RESTART=1)" << endl;
      }
...
void System :: initialize_velocities(){
  ...
  if(_restart){ // Read the previous configuration from conf-1.xyz
    ifstream cinf;
    ...
    if(cinf.is_open()){
      ...
      int ncoord;
      cinf >> ncoord;
      if (ncoord != _npart){ // Check if the number of coordinates in conf-1.xyz matches the number of particles in input.dat
        cerr << "PROBLEM: conflicting number of coordinates in input.dat & config.xyz not match!" << endl;
        exit(EXIT_FAILURE);
      }
      cinf >> comment;
      for(int i=0; i<_npart; i++){ // Read the coordinates of the particles from conf-1.xyz and set them in the Particle objects
        cinf >> particle >> xold >> yold >> zold; // units of coordinates in conf.xyz is _side
        // Convert coordinates from reduced units to simulation units
        xold = this->pbc(_side(0) * xold, 0);
        yold = this->pbc(_side(1) * yold, 1);
        zold = this->pbc(_side(2) * zold, 2);

        if(_reverse){

          // Reverse the direction of time:
          // x_old(reverse) = 2*x(current) - x_old(forward)

          xold = this->pbc(
              2.0 * _particle(i).getposition(0,true) - xold, 0);

          yold = this->pbc(
              2.0 * _particle(i).getposition(1,true) - yold, 1);

          zold = this->pbc(
              2.0 * _particle(i).getposition(2,true) - zold, 2);
    ...
    cinf.close();
  } else if (_start_velocities == "MAXBOLTZ"){ // Initialize the velocities of the particles according to a Maxwell-Boltzmann distribution
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();
    for (int i=0; i<_npart; i++){ // Assign random velocities to the particles according to a Gaussian distribution with mean 0 and variance _temp
    ...
    for (int idim=0; idim<_ndim; idim++) sumv(idim) = sumv(idim)/double(_npart); // Compute the average velocity of the system along each dimension
    double sumv2 = 0.0, scalef;
    for (int i=0; i<_npart; i++){ // Subtract the average velocity from each particle's velocity to ensure the center of mass is at rest
      vx(i) = vx(i) - sumv(0);
      vy(i) = vy(i) - sumv(1);
      vz(i) = vz(i) - sumv(2);
      sumv2 += vx(i) * vx(i) + vy(i) * vy(i) + vz(i) * vz(i);
    }
    sumv2 /= double(_npart); // Compute the average squared velocity of the system
    scalef = sqrt(3.0 * _temp / sumv2);   // velocity scale factor 
    for (int i=0; i<_npart; i++){ // Scale the velocities of the particles to match the desired temperature and set them in the Particle objects
      _particle(i).setvelocity(0, vx(i)*scalef);
      _particle(i).setvelocity(1, vy(i)*scalef);
      _particle(i).setvelocity(2, vz(i)*scalef);
    }
    for (int i=0; i<_npart; i++){ // Compute the previous positions of the particles based on their current positions and velocities, and set them in the Particle objects
    ...
    if (_npart % 2 != 0){ // Check if the number of particles is even, as the Dirac distribution requires pairs of particles with opposite velocities
      cerr << "PROBLEM: DIRAC distribution is only implemented for an even number of particles" << endl;
      exit(EXIT_FAILURE);
    }
    vec vx(_npart), vy(_npart), vz(_npart);
    double v0 = sqrt(3.0 * _temp); // Compute the velocity magnitude corresponding to the desired temperature
    for (int i = 0; i < _npart; i += 2){ 
      vx(i) = v0; vy(i) = 0.0; vz(i) = 0.0; // Assign velocity to particle i
      vx(i + 1) = -v0; vy(i + 1) = 0.0; vz(i + 1) = 0.0; // Assign opposite velocity to particle i+1
    }
    for (int i = 0; i < _npart; i++){ // Set the velocities in the Particle objects
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutp.close();
        ofstream coutpi("../OUTPUT/instantaneous_potential_energy.dat");
        coutpi << "#  U/N:" << endl;
        coutpi.close();
        _nprop++;
        ...
        coutte.close();
        ofstream coutti("../OUTPUT/instantaneous_temperature.dat");
        coutti << "# T" << endl;
        coutti.close();
        _nprop++;
        _measure_temp = true;
        ...
        index_property++;
        _ptail = (32.0 * M_PI * _rho / 9.0) * 1.0 / pow(_r_cut, 9) - (16.0 * M_PI * _rho / 3.0) * 1.0 / pow(_r_cut, 3);
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.dat");
        coutgr << "# BLOCK:     DISTANCE:    ACTUAL_GOFR:    GOFR_AVE:    ERROR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // 4*sigma_v is the maximum velocity considered for the histogram
        _measure_pofv = true;
...
void System :: finalize(){
  ...
  ofstream coutf;
  if (_measure_gofr){
    coutf.open("../OUTPUT/gofr_final.dat", ios::out);
    coutf << "# DISTANCE:     GOFR:        ERROR:" << endl;
    for(int i=0; i<_n_bins; i++){
      double r = (i + 0.5) * _bin_size;
      double sum_average = _global_av(_index_gofr + i);
      double sum_ave2    = _global_av2(_index_gofr + i);
      double average = sum_average / double(_nblocks);
...
void System :: write_XYZ(int nconf){
  cout << "Configuration n. " << nconf << " written in file config_" << nconf << ".xyz" << endl;
  ofstream coutf;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        // GOFR conteggio delle coppie di particelle a distanza dr
        if (_measure_gofr && dr < _side(0)/2.0){
          bin = static_cast<int>(dr / _bin_size);

          if (bin >= 0 && bin < _n_bins)
            _measurement(_index_gofr + bin) += 1.0;
        }
        if(dr < _r_cut){
  ...
  if (_measure_gofr){
    for (int i=0; i<_n_bins; i++){
      ...
      double shell_volume =
        (4.0 * M_PI / 3.0) *
        (pow(r + _bin_size/2.0, 3) -
         pow(r - _bin_size/2.0, 3));

      _measurement(_index_gofr + i) *=
        2.0 / (_npart * _rho * shell_volume);
    }
  }
  if (_measure_pofv) {
    for (int i = 0; i < _n_bins_v; i++) _measurement(_index_pofv + i) = 0.0; // Reset the histogram for POFV
    for (int i = 0; i < _npart; i++) {
      double vx = _particle(i).getvelocity(0);
      double vy = _particle(i).getvelocity(1);
      double vz = _particle(i).getvelocity(2);
      double v = sqrt(vx * vx + vy * vy + vz * vz); // Compute the speed of particle i
      bin = static_cast<int>(v / _bin_size_v); // Determine the bin index for the speed
      if (bin >= 0 && bin < _n_bins_v) _measurement(_index_pofv + bin) += 1.0; // Increment the histogram count for the corresponding bin
    }
    for (int i = 0; i < _n_bins_v; i++) _measurement(_index_pofv + i) /= (_npart * _bin_size_v); // Normalize the histogram 
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    ...
    ofstream coutf("../OUTPUT/instantaneous_potential_energy.dat", ios::app);
    coutf << setprecision(12) << penergy_temp << endl;
    coutf.close();
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
  ...
  if (_measure_temp and _measure_kenergy){
    ...
    ofstream coutf("../OUTPUT/instantaneous_temperature.dat", ios::app);
    coutf << setprecision(12) << _measurement(_index_temp) << endl;
    coutf.close();
  }
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure){
    ...
    if (_sim_type == 0)
        ideal_term = _rho * (2.0/3.0) * kenergy_temp;  // MD
    else
        ideal_term = _rho * _temp;                     // MC
...
void System :: averages(int blk){
  ...
  if(_measure_cv){
    ...
    _average(_index_cv) = _beta * _beta * double(_npart) * (e2_ave - e_ave*e_ave);
  }
  _global_av  += _average;
  ...
  if (_measure_pofv){
    ...
    for(int i=0; i<_n_bins_v; i++){ 
      average  = _average(_index_pofv + i); // Average value of the i-th bin of the POFV histogram for the current block
    ...
    coutf << endl; // Add an empty line to separate blocks in the output file
    coutf.close();
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
