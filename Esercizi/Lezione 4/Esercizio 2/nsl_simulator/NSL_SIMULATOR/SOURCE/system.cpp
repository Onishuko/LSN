...
// Write the initial configuration of the velocities
void System::write_initial_pofv(){ 

  ofstream coutpv("../OUTPUT/initial_pofv.dat");

  if(!coutpv.is_open()){
    cerr << "PROBLEM: Unable to open initial_pofv.dat" << endl;
    return;
  }

  vec histogram(_n_bins_v);
  histogram.zeros();
  ...
  for(int i = 0; i < _npart; i++){
    ...
    int bin = static_cast<int>(v / _bin_size_v);

    if(bin >= 0 && bin < _n_bins_v)
      histogram(bin) += 1.0;
  }

  histogram /= (_npart * _bin_size_v);

  coutpv << "# VELOCITY: INITIAL_POFV:" << endl;

  for(int i = 0; i < _n_bins_v; i++){

    double v = (i + 0.5) * _bin_size_v;

    coutpv << setprecision(17)
           << v << " "
           << histogram(i) << endl;
  }
